...
model_data = joblib.load('output_models/weighted.joblib')
...
def main():
    while True:
        file_path = input("\nEnter the path to the audio file (or 'q' to quit): ")

        if file_path.lower() == 'q':
            break
        
        try:
            probability = predict_sample(file_path)
            
            print("\nPrediction Results:")
            prediction = "Spoof" if probability > best_threshold else "Bonafide"
